...
/home/qemu-2021/build/qemu-system-arm -m 1024 -M virt -name guest=aa32 -kernel bin/BOOTAA32.BIN -smp 2,sockets=1,threads=1 -uuid 515645b7-ab3a-4e82-ba62-25751e4b523f -m 1G -device bochs-display -display gtk -monitor stdio -drive file=/home/images/gigdisk.img
...
/home/qemu-2021/build/qemu-system-riscv32 -name guest=rv32 -M virt  -bios bin/rv32fw_jump.elf -kernel bin/BOOTRV32.BIN -append "root=/dev/vda ro" -netdev user,id=net0 -device virtio-net-device,netdev=net0 -device bochs-display -display gtk -monitor stdio -device virtio-blk-device,drive=hd0 -drive file=/home/images/gigdisk.img,format=raw,id=hd0
